...
(function () {
    ...
    // Short descriptions under each card title (Family-style stacked cards).
    var SOCIAL_DESC = {
        profile:   'My Sarawak Talents name card',
        website:   'Visit my website',
        instagram: 'Follow me on Instagram',
        x:         'Follow me on X',
        linkedin:  'Connect with me on LinkedIn',
        facebook:  'Find me on Facebook',
        tiktok:    'Watch me on TikTok',
    ...
    function buildInteractiveQR(p) {
        ...
        // Cards carry an icon + title + description (Family-style on phones).
        buttons.innerHTML = '';
        [PROFILE_KEY].concat(connected).forEach(function (k) {
            var meta = metaFor(k);
            var desc = SOCIAL_DESC[k] || '';
            ...
            b.innerHTML = '<span class="qr-social-ic" style="background:' + meta.color + '">' + iconHTML(meta) + '</span>' +
                          '<span class="qr-social-text">' +
                              '<span class="qr-social-label">' + meta.label + '</span>' +
                              (desc ? '<span class="qr-social-desc">' + desc + '</span>' : '') +
                          '</span>' +
